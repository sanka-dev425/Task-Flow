...
export default function LandingPage() {
  return (
    <div className="min-h-[100dvh] bg-background text-foreground font-sans relative overflow-hidden flex flex-col selection:bg-primary selection:text-primary-foreground">
      ...
      <header className="relative z-10 container mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <SignUpButton mode="modal">
            <Button className="bg-primary hover:bg-primary/90 text-white rounded-full px-6 font-semibold shadow-md hover:shadow-lg transition-all">
              Start Free
            </Button>
          </SignUpButton>
        </div>
      ...
      {/* Hero Section */}
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 py-20 pb-32">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-secondary/50 border border-border/50 text-sm font-semibold text-primary mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <Sparkles className="h-4 w-4" />
          The workspace for ambitious minds
        </div>
        
        <h1 className="font-serif text-5xl md:text-7xl font-bold tracking-tight max-w-4xl leading-[1.1] mb-6 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-100 fill-mode-both">
          Clarity in the chaos. <br className="hidden md:block" />
          Focus on what matters.
        </h1>
        
        <p className="text-xl md:text-2xl text-muted-foreground max-w-2xl font-light mb-10 leading-relaxed animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 fill-mode-both">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 animate-in fade-in slide-in-from-bottom-10 duration-700 delay-300 fill-mode-both">
          <SignUpButton mode="modal">
            <Button className="h-14 px-8 rounded-full bg-primary hover:bg-primary/90 text-white text-lg font-semibold shadow-[0_8px_20px_-4px_rgba(37,99,235,0.4)] hover:shadow-[0_12px_24px_-4px_rgba(37,99,235,0.5)] transition-all group">
              Start your journey
              <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
            </Button>
          </SignUpButton>
          <SignInButton mode="modal">
            <Button variant="outline" className="h-14 px-8 rounded-full text-lg font-semibold bg-white/50 backdrop-blur-sm border-border hover:bg-white hover:text-foreground transition-all">
        ...
        {/* Feature Highlights */}
        <div className="mt-24 grid sm:grid-cols-3 gap-8 max-w-4xl text-left animate-in fade-in slide-in-from-bottom-12 duration-700 delay-500 fill-mode-both">
          {[
            {
